<template>
	<view>
			...
			<view class="uni-margin-wrap">
				<swiper class="swiper" circular :indicator-dots="true" :autoplay="true" :interval="3000" :duration="500">
					<swiper-item v-for="(item, index) in imgList" :key="index">
						<view class="swiper-item uni-bg-red"><image style="width: 100%;" :src="item.url" mode="aspectFill"></image></view>
					</swiper-item>
				</swiper>
			</view>
		</view>
		...
		<view class="d-flex product-list">
			<view class="product-item" v-for="(v, i) in goodList" :key="i">
...
export default {
	...
	methods: {
		...
		async getImgs() {
			let { code, data } = await http('GET', 'api/img');
			this.imgList = data
		...
		async getGoods() {
			let { data } = await http('GET', 'api/products');
			this.goodList = data
				.map(v => ({
					...v,
